...
class Adult(Patient):
    def __init__(self,id,name,age,gender,citizen_status,medisave_balance):
        super().__init__(id,name,age,gender,citizen_status)
        self.__medisave_balance=medisave_balance

    @property
    def medisave_balance(self):
        return self.__medisave_balance

    @medisave_balance.setter
    def medisave_balance(self,medisave_balance):
        self.__medisave_balance = medisave_balance
    ...
    def calculate_charges(self):
        ...
        for i,each in enumerate(self.stay_list):
            if each.payment_status=='Unpaid':
                ...
                for n,every in enumerate(each.bedstay_list):
                    ...
                    total+=sub
        if self.__medisave_balance < total:
            subtotal=total-self.__medisave_balance
            deduct=self.__medisave_balance
        else:
            subtotal=0
            deduct=total
        print('=========')
        print('Total charges pending:$',total)
        print('Medisave balance:$',self.__medisave_balance)
        print('To deduct from Medisave:$',deduct)
        ...
        if payment=='Y':
            ...
            print('\n$ {:d} has been deducted from MediSave'.format(deduct))
            self.__medisave_balance=self.__medisave_balance-deduct
            print('New Medisave balance:$',self.__medisave_balance)
    ...
    def __repr__(self):
        return self.__class__.__name__ + "('{:s}','{:s}','{:d}','{:s}','{:s}','{:d}')".format(
            self.id,self.name,self.age,self.gender,self.citizen_status,self.__medisave_balance)
